...
export function DashboardPage() {
  ...
  useEffect(() => {
    async function loadDashboardData() {
      try {
        setIsLoading(true);
        const [memberCount, todayCheckins, todayRecords, expiring] = await Promise.all([memberService.getCount(), attendanceService.getTodayCount(), attendanceService.getTodayRecords(), membershipService.getExpiringSoon(7)]);
        setStats({
  ...
  return <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">{"dashboard.hello"}</h1>
