...
export function MemberDetailPage() {
  ...
  useEffect(() => {
    async function fetchMember() {
      ...
      try {
        setIsLoading(true);
        const [memberData, membershipData, daysData, historyData] = await Promise.all([memberService.getById(id), membershipService.getActiveMembership(id), memberService.getTrainingDays(id), attendanceService.getMemberHistory(id)]);
  ...
  const handleRefresh = () => {
    if (id) {
      setIsLoading(true);
      Promise.all([memberService.getById(id), membershipService.getActiveMembership(id), memberService.getTrainingDays(id), attendanceService.getMemberHistory(id)]).then(([memberData, membershipData, daysData, historyData]) => {
        setMember(memberData);
        setActiveMembership(membershipData);
        setTrainingDays(daysData);
        setAttendanceHistory(historyData);
  ...
  return <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/members')} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title={"Membresía Actual"}>
          {activeMembership ? <div className="space-y-4">
            ...
            </div> : <div className="py-8 text-center text-slate-500 flex flex-col items-center">
        ...
        <Card title={"Historial de Asistencia"}>
          {attendanceHistory.length > 0 ? <div className="space-y-3">
              {attendanceHistory.slice(0, 5).map(record => <div key={record.id} className="flex justify-between items-center p-3 border border-slate-100 rounded-xl bg-slate-50">
                  <div>
                  ...
                  locale: dateLocale
                })} • {record.status === 'valid' ? "Permitido" : record.status === 'denied' ? "Denegado" : "Duplicado"}
                    </div>
                  </div>
                  <div className={`w-3 h-3 rounded-full ${record.status === 'valid' ? 'bg-green-500' : record.status === 'denied' ? 'bg-red-500' : 'bg-yellow-500'}`} />
                </div>)}
              {attendanceHistory.length > 5 && <div className="text-center pt-2">
      ...
      <AssignMembershipModal isOpen={isAssignModalOpen} onClose={() => setIsAssignModalOpen(false)} memberId={member.id} onSuccess={handleRefresh} />
    </div>;
}