...
export function MemberProfile({ member, trainingDays = [], attendanceHistory = [] }: MemberProfileProps) {
  const variants = {
    active: 'success',
    expired: 'danger',
    suspended: 'warning',
  ...
  const validAttendances = attendanceHistory.filter(r => r.status === 'valid' || r.status === 'manual')
