...
export function ReportsDashboardPage() {
  ...
  const { data, isLoading, error } = useReport(
    () =>
      activeTenantId
        ? getDashboardSummary(activeTenantId, range)
  ...
  return (
    <div className="space-y-6">
      ...
      <ReportHeader
        ...
        subtitle={'Visión general del rendimiento del gimnasio'}
        preset={preset}
        range={range}
        actions={
          <ExportPdfButton
            ...
            filename={`reporte-general-${range.from}`}
          />
        }
      />
      ...
      <ReportSummaryGrid columns={4}>
        <ReportCard
          ...
          value={data ? formatMXN(data.totalRevenue) : '—'}
        ...
        <ReportCard
          ...
          color="success"
          loading={isLoading}
          subtitle={data ? 'reports.dashboard.avgPerDay' : undefined}
        />
        <ReportCard
        ...
        <ReportCard
          ...
          value={data ? `${data.avgDailyAttendances} / día` : '—'}
          icon={<TrendingUp className="w-5 h-5" />}
          color="neutral"
          loading={isLoading}
          subtitle={'Asistencias válidas'}
        />
        <ReportCard
          title={'Plan principal'}
          value={data?.topPlan?.name ?? '—'}
          icon={<Star className="w-5 h-5" />}
          color="primary"
          loading={isLoading}
          subtitle={data?.topPlan ? `${data.topPlan.count} membresías` : undefined}
        />
      </ReportSummaryGrid>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ReportChartCard
          ...
          subtitle={'Evolución de ingresos en el periodo seleccionado'}
        >
          <LineChartReport
