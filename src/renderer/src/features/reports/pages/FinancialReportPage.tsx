...
export function FinancialReportPage() {
  ...
  const paymentColumns: TableColumn<PaymentRow>[] = [
    ...
    },
    {
      ...
      align: 'right',
      render: (row) => (
        <span className="font-semibold text-emerald-700">{formatMXN(row.amount)}</span>
      ),
      width: '120px',
    },
  ]
  ...
  const { data, isLoading, error } = useReport(
    () =>
      activeTenantId
        ? getFinancialReport(activeTenantId, range)
    ...
    <div className="space-y-6">
        ...
        actions={
          <div className="flex items-center gap-2">
            <ExportCsvButton
              columns={csvColumns}
              data={data?.payments ?? []}
              filename={`financiero-${range.from}`}
            />
            <ExportPdfButton
              meta={pdfMeta}
      ...
      <ReportSummaryGrid columns={4}>
        <ReportCard
          ...
          value={data ? formatMXN(data.summary.totalRevenue) : '—'}
        ...
        <ReportCard
          ...
          value={data ? formatMXN(data.summary.avgTicket) : '—'}
        ...
        <ReportCard
          ...
          value={data ? formatMXN(data.summary.totalDiscounts) : '—'}
